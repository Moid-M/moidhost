...
async function loadStats() { try { const [sys, srv] = await Promise.all([api('/system/stats'), selectedId ? api('/servers/'+selectedId+'/stats') : null]); const now = Date.now(); let cpuPct, memPct, memTotal, diskPct, diskTotal, memBytes, diskBytes; if (srv && srv.process_pid > 0) { cpuPct = srv.cpu_percent; memBytes = srv.memory_rss_bytes; memTotal = sys.memory.total_bytes; memPct = memTotal > 0 ? Math.round(srv.memory_rss_bytes / memTotal * 1000) / 10 : 0; diskBytes = srv.disk_used_bytes; diskTotal = srv.disk_limit_bytes > 0 ? srv.disk_limit_bytes : sys.disk.total_bytes; diskPct = diskTotal > 0 ? Math.round(srv.disk_used_bytes / diskTotal * 1000) / 10 : 0; } else { cpuPct = sys.cpu.percent; memBytes = sys.memory.used_bytes; memTotal = sys.memory.total_bytes; memPct = sys.memory.percent; diskBytes = sys.disk.used_bytes; diskTotal = sys.disk.total_bytes; diskPct = sys.disk.percent; } statsCpu.push({t:now,v:cpuPct}); statsMem.push({t:now,v:memPct}); statsDisk.push({t:now,v:diskPct}); if(statsCpu.length>120) statsCpu.shift(); if(statsMem.length>120) statsMem.shift(); if(statsDisk.length>120) statsDisk.shift(); drawGraph($('#stats-cpu'),'CPU','#ff6b9d',statsCpu); drawGraph($('#stats-mem'),'RAM','#3fb950',statsMem,memTotal); drawGraph($('#stats-disk'),'Disk','#58a6ff',statsDisk,diskTotal); } catch(_) {} }
